<?php
  ...
  $headers .= 'From: billetterieUTC-billetterie@assos.utc.fr'."\r\n";
  $headers .= 'Reply-To: billetterieUTC-billetterie@assos.utc.fr'."\r\n";
    ...
    foreach ($transactions as $transaction) {
      ...
      if ($info['status'] == 'V') {
        ...
        if ($query->rowCount() == 0) {
          ...
          mail('billetterieUTC@assos.utc.fr', 'Billet possiblement payé mais non comptabilisé !',
"Coucou,<br />
<br />
...
Le bot de Samy", $GLOBALS['headers']);
          ...
          $url = 'http://'.$_SERVER['HTTP_HOST']."/billetterieUTC/account.php";

          mail($_SESSION['email'], 'Confirmation de réservation billetterieUTC',
"Bonjour,<br />
<br />
...
L'équipe de billetterieUTC.", $headers);
  ...
  function generateTransaction() {
    ...
    if ($query->rowCount() != 0) {
      ...
      $transaction = $payutc->createTransaction($items, $_SESSION['email'], 'http://'.$_SERVER['HTTP_HOST'].'/billetterieUTC/pay.php');
