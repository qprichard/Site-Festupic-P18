  ...
  if ($query->rowCount() == 0 || isset($_GET['consos'])) {
    if (isset($_SESSION['consos']) && isset($_SESSION['consos']['idTransaction'])) {
      ...
      elseif ($info['status'] == 'V') {
        ...
        $url = 'https://'.$_SERVER['HTTP_HOST']."/billetterieUTC/consos.php";

        mail($_SESSION['email'], 'Confirmation d\'achat de consos: billetterieUTC',
"Bonjour,<br />
<br />
...
L'équipe de billetterieUTC.", $headers);
  ...
  elseif ($info['status'] == 'V') {
    ...
    $url = 'https://'.$_SERVER['HTTP_HOST']."/billetterieUTC/account.php";

    mail($_SESSION['email'], 'Confirmation d\'achat de votre/vos billet(s): billetterieUTC',
"Bonjour,<br />
<br />
